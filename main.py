...
def get_model_fn(num_gpus, variable_strategy, num_workers):
  ...
  def _model_fn(features, labels, mode, params):
        ...
        with tf.name_scope('tower_%d' % i) as name_scope:
          with tf.device(device_setter):
            loss, gradvars, preds = _tower_fn(
                is_training, params.dp_keep_prob, weight_decay, tower_features[i], tower_labels[i],
    ...
    with tf.device(consolidation_device):
      ...
      tensors_to_log = {'learning_rate': learning_rate, 'loss': loss, 'acc': metrics['accuracy'][0]}
...
def _tower_fn(is_training, dp_keep_prob, weight_decay, feature, label, data_format,
              num_layers, batch_norm_decay, batch_norm_epsilon, params):

  if params.model_name == 'mobilenet_v2':
    with slim.arg_scope(mobilenet_v2.training_scope(is_training=True, dropout_keep_prob=dp_keep_prob)):
      logits, end_points = mobilenet_v2.mobilenet(feature, num_classes=num_classes, prediction_fn=None)
  if params.model_name == 'mobilenet_v1':
    with slim.arg_scope(mobilenet_v1.mobilenet_v1_arg_scope(is_training=False)):
      logits, end_points = mobilenet_v1.mobilenet_v1(feature, num_classes=num_classes)
  ...
  if params.weight_decay_not_used_on_depthwise:
    model_params_for_weight_decay = [v for v in model_params if v not in depthwise_params]
  else:
...
def get_experiment_fn(data_dir,
  ...
  def _experiment_fn(run_config, hparams):
    ...
    num_eval_examples = imagenet.ImageNetDataSet.num_examples_per_epoch('eval')
...
if __name__ == '__main__':
  ...
  parser.add_argument('--learning-rate', type=float, default=0.1, help="""\
      ...
      this file.\
      """)
  parser.add_argument('--use-distortion-for-training', type=bool, default=True,
          help='If doing image distortion for training.')
  parser.add_argument('--sync', action='store_true', default=False, help="""\
      If present when running in a distributed environment will run on sync mode.\
      """)
  parser.add_argument('--num-intra-threads', type=int, default=0,
      help="""\
      Number of threads to use for intra-op parallelism. When training on CPU
      set to 0 to have the system pick the appropriate number or alternatively
      set it to the number of physical CPU cores.\
      """)
  parser.add_argument('--num-inter-threads', type=int, default=0,
      help="""\
  ...
  parser.add_argument('--dp_keep_prob', type=float, default=0.8, help='dp_keep_prob.')
  #parser.add_argument('--change_lr_epoch_size', type=int, default=1, help='change_lr_epoch_size')
  #parser.add_argument('--Optimizer', type=str, default='SGD', help='SGD or RMSPROP')
  parser.add_argument('--weight_decay_not_used_on_depthwise', type=int, default=0, help='0 / 1.')
